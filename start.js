...
document.addEventListener('DOMContentLoaded', async function() {
  ...
  // Set up footer menu functionality for start screen
  setupStartScreenFooterMenus();
});

// Global variable for action mode
let mode = null;

// Setup footer menu functionality for start screen
function setupStartScreenFooterMenus() {
...
async function handleVocabMenuClick(event) {
  // Only show menu if we're on start screen
  if (!document.querySelector('.screen-start.active')) return;
...
async function showVocabMenu() {
  ...
  await showPopupMenu(
    ...
    (selectedItem) => {
      mode = selectedItem;
      showScreen('f2'); // Navigate to action screen
    }
  );
}
  ...
  await showPopupMenu(
    ...
    (selectedItem) => {
      if (selectedItem === 'profile') {
        showScreen('f3'); // Navigate to profile screen
      } else if (selectedItem === 'settings') {
        // Handle settings - could show settings screen or modal
        // For now, just log the action
        console.log('Settings selected - functionality to be implemented');
      }
    }
  );
}
